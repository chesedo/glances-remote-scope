...
ResourcesCategory.prototype.createDiskIOCard = function (category) {
    ...
    categorised_result.set("total", total);
    categorised_result.set("diskio", data.getValue("diskio"));
...
ResourcesCategory.prototype.createPreview = function(preview_reply, result) {
    switch (result.get("uri")) {
        ...
        case "Memory":
        case "Swap":
            this.createGeneralPreview(preview_reply, result);
            break;
        case "Disk IO":
...
ResourcesCategory.prototype.createGeneralPreview = function(preview_reply, result) {
...
ResourcesCategory.prototype.createDiskIOPreview = function(preview_reply, result) {
    ...
    var tableData = [
        ["Total", result.get("attributes")[0].value],
        ["Read", data.convertByteRate(total["read_bytes"])],
        ["Write", data.convertByteRate(total["write_bytes"])]
    ]
    table.add_attribute_value("values", tableData);

    // Create expandable
    var exp = new scopes.lib.PreviewWidget("exp", "expandable");
    var diskio = result.get("diskio");

    // Add each disk as table to expandable
    diskio.forEach(function(disk) {
        var diskTable = new scopes.lib.PreviewWidget(disk["disk_name"], "table");
        var diskTableData = [
        ...
        diskTable.add_attribute_value("title", disk["disk_name"]);
        diskTable.add_attribute_value("values", diskTableData);
